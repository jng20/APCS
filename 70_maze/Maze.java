...
class MazeSolver
{
  ...
  public void solve( int x, int y )
  {
    ...
    //primary base case
    if ( _maze[y][x] == (EXIT) ) { //base case of being at the end
        _solved = true;
         System.exit(0);
    }
    //other base cases
    else if ( !(_maze[y][x] ==(PATH)) ) { //the space doesn't exist. exhaust all possible paths.  _maze[y][x].equals(" ") || _maze[y][x].equals(".")
	//???
      return;
    }
...
//???    stepped on twice so set to dot
      if(_maze[y][x]==(HERO)){
